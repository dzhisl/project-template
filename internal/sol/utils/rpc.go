...
func ConfirmTransaction(signature solana.Signature) error {
	...
	for {
		...
		case <-ticker.C:
			logger.Debug("polling transaction", zap.String("sig", signature.String()))
			...
			logger.Debug("poll res", zap.Any("", txResult))
			if err != nil {
				logger.Error("error in getting signature status", zap.Error(err))
